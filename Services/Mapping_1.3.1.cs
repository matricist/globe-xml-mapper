        ...
        )
        {
            ...
            foreach (var blockStartRow in blockStartRows)
            {
                ...
                foreach (var (offset, target) in FieldMap)
                {
                    ...
                    if (target == "Upe.Art1035")
                    {
                        art1035Raw = cellValue;
                        continue;
                    }
                    if (target == "Upe.ExcludedUpe.ExcludedUpeStatus")
                        hasExcludedStatus = true;

                    var isMulti =
                        target is "Upe.OtherUpe.Id.Rules" or "Upe.OtherUpe.Id.GlobeStatus";
                    var values = isMulti
                        ? cellValue.Split(
                            ',',
                    ...
                    foreach (var val in values)
                    {
                        ...
                        else if (target.StartsWith("Upe.ExcludedUpe."))
                        {
                            ...
                            SetExcludedUpe(
                                upe.ExcludedUpe,
                                target.Replace("Upe.ExcludedUpe.", ""),
                                val,
                                errors,
                                fileName,
                                entry
                ...
                if (hasExcludedStatus && upe.OtherUpe?.Id != null)
                {
                    ...
                    if (src.KName != null)
                        dst.KName = src.KName;
                    foreach (var v in src.ResCountryCode)
                        dst.ResCountryCode.Add(v);
                    foreach (var v in src.Tin)
                        dst.Tin.Add(v);
                    foreach (var v in src.Rules)
                        dst.Rules.Add(v);
                    foreach (var v in src.GlobeStatus)
                        dst.GlobeStatus.Add(v);
                    if (dst.Tin.Count == 0)
                        dst.Tin.Add(NoTin());
                    upe.OtherUpe = null;
                }
                ...
                if (art1035Raw != null)
                {
                    ...
                    if (hasExcludedStatus)
                    {
                        ...
                        SetEnum<Globe.CountryCodeType>(
                            ...
                            v =>
                            {
                                upe.ExcludedUpe.Art1035 = v;
                                upe.ExcludedUpe.Art1035Specified = true;
                            },
                            errors,
                            fileName,
                            entry
                    ...
                    else
                    {
                        ...
                        SetEnum<Globe.CountryCodeType>(
                            ...
                            v =>
                            {
                                upe.OtherUpe.Art1035 = v;
                                upe.OtherUpe.Art1035Specified = true;
                            },
                            errors,
                            fileName,
                            entry
        ...
        )
        {
            switch (path)
            {
                ...
                case "Id.GlobeStatus":
                    ...
                    break;
                default:
                    errors.Add($"[{fileName}] 셀 {entry.Cell}: UPE 알 수 없는 경로 '{path}'");
                    break;
            }
        }

        private void SetExcludedUpe(
            Globe.CorporateStructureTypeUpeExcludedUpe excUpe,
            string path,
            string value,
            List<string> errors,
            string fileName,
        ...
        )
        {
            switch (path)
            {
                case "ExcludedUpeStatus":
                    SetEnum<Globe.ExcludedUpeEnumType>(
                        value,
                        v => excUpe.ExcludedUpeStatus = v,
                        errors,
                        fileName,
                        entry
                    );
                ...
                default:
                    errors.Add(
                        $"[{fileName}] 셀 {entry.Cell}: ExcludedUPE 알 수 없는 경로 '{path}'"
                    );
                    break;
            }
        }
