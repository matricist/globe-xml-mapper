...
namespace GlobeMapper.Services
{
    ...
    public static class ValidationUtil
    {
            ...
            foreach (var upe in corpStructure.Upe)
            {
                if (upe.OtherUpe?.Id?.Tin != null)
                {
                    foreach (var tin in upe.OtherUpe.Id.Tin)
                        ValidateSingleTin(tin, "OtherUPE", true, errors);
                }
                if (upe.ExcludedUpe?.Id?.Tin != null)
                {
                    foreach (var tin in upe.ExcludedUpe.Id.Tin)
                        ValidateSingleTin(tin, "ExcludedUPE", true, errors);
                }
            }
        ...
        private static void ValidateSingleTin(Globe.TinType tin, string context, bool isUpe, List<string> errors)
        {
            ...
            // 70002: TIN='NOTIN'이면 GIR3004, Unknown=TRUE
            if (isNoTin && (!isGir3004 || !isUnknown))
            {
                errors.Add($"[70002] {context} TIN='NOTIN'이면 TypeOfTIN=GIR3004, Unknown=TRUE이어야 합니다.");
            }

            // 70003: Unknown=TRUE이면 NOTIN, GIR3004
            if (isUnknown && (!isNoTin || !isGir3004))
            {
                errors.Add($"[70003] {context} Unknown=TRUE이면 TIN='NOTIN', TypeOfTIN=GIR3004이어야 합니다.");
            }
